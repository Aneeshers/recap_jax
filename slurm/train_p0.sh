#!/bin/bash
#SBATCH --job-name=recap2_p0                   # Default job name (sweep overrides)
...
#SBATCH -o slurm-%x-%A_%a.out                 # STDOUT (%x=jobname, %A=array_job_id, %a=task_id)
#SBATCH -e slurm-%x-%A_%a.err                 # STDERR
...
~/.conda/envs/torch/bin/python phase-zero.py --use_lambda_returns
