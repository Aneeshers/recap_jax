...
"""
RECAP Phase 0 + Chunk-Level Validation Diagnostics

Adds:
1) H-step advantage diagnostics
2) Chunked execution evaluation
"""
...
def step_env(env, action):
    ...
    if len(out) == 5:
        ...
        return obs, float(r), terminated or truncated, info
    obs, r, done, info = out
...
def compute_rtg(rewards, dones, gamma):
...
def compute_h_step_adv(rews, dones, v_pred, gamma, H):
    n = len(rews)
    adv_h = np.zeros(n, dtype=np.float32)
    ...
    for t in range(n):
        ...
        for k in range(H):
            if t + k >= n:
                break
            ret += discount * rews[t + k]
            if dones[t + k]:
                break
            discount *= gamma

        if t + H < n:
            ret += discount * v_pred[t + H]

        adv_h[t] = ret - v_pred[t]
...
def main():
    ...
    p.add_argument("--chunk_sizes", type=str, default="1,5,10",
               help="Comma-separated chunk sizes for evaluation")
    ...
    # ===== Chunk Advantage Diagnostics =====
    chunk_sizes = [int(x) for x in args.chunk_sizes.split(",")]

    for H in chunk_sizes:
        adv_h = compute_h_step_adv(rews, dones, v_pred, args.gamma, H)

        wandb.log({
            f"chunk_adv/H{H}_std": float(adv_h.std()),
            f"chunk_adv/H{H}_mean": float(adv_h.mean()),
            f"chunk_adv/H{H}_pos_frac": float((adv_h > 0).mean()),
            f"chunk_adv/H{H}_corr_with_rtg":
                float(np.corrcoef(adv_h, rtg)[0,1]),
        }, step=adv_step)

        print(f"  H={H}: std={adv_h.std():.4f} "
            f"corr={np.corrcoef(adv_h, rtg)[0,1]:.3f}")
    ...
    print("\n[Chunked Execution Evaluation]")

    for H in chunk_sizes:
        avg_ret, avg_succ = evaluate_chunked(
            env,
            flow_actor,
            actor_state.params,
            args.eval_episodes,
        ...
        wandb.log({
            f"eval_chunk/H{H}_return": avg_ret,
            f"eval_chunk/H{H}_success": avg_succ,
        }, step=final_step)

        print(f"  Chunk H={H}: return={avg_ret:.2f} "
            f"success={avg_succ:.2%}")
